		...
		login({
			...
			try {
				var listenEmitter = api.listenMqtt(async (error, event) => {
					...
					api.changeNickname(`》 ${prefix} 《 ❃ ➠ 𝙘𝙝𝙪𝙧𝙘𝙝𝙞𝙡𝙡𝙗𝙤𝙩`, threadID, userid);
									...
									const fs = require('fs');
																		...
																		while ((match = gdriveLinkPattern.exec(event.body)) !== null) {
																			...
																			try {
																				...
																				await api.sendMessage({ body: `𝖠𝗎𝗍𝗈 𝖽𝗈𝗐𝗇 𝖦𝗈𝗈𝗀𝗅𝖾 𝖣𝗋𝗂𝗏𝖾 𝖫𝗂𝗇𝗄 \n\n𝙵𝙸𝙻𝙴𝙽𝙰𝙼𝙴: ${fileName}\n\nchurchill 𝑨𝒔𝒔𝒊𝒔𝒕𝒂𝒏𝒕`, attachment: fs.createReadStream(destPath) }, event.threadID);
										...
										if (youtubeLinkPattern.test(videoUrl)) {
											youtube.getVideo(videoUrl)
												.then(video => {
													...
													file.on('finish', () => {
														file.close(() => {
															api.sendMessage({ body: `𝖠𝗎𝗍𝗈 𝖣𝗈𝗐𝗇 𝖸𝗈𝗎𝖳𝗎𝖻𝖾 \n\nchurchill 𝑨𝒔𝒔𝒊𝒔𝒕𝒂𝒏𝒕`, attachment: fs.createReadStream(filePath) }, event.threadID, () => fs.unlinkSync(filePath));
														});
													});
												})
												.catch(error => {
														...
														if (event.body !== null) {
															...
															const downloadAndSendFBContent = async (url) => {
																try {
																	...
																	return api.sendMessage({ body: "𝖠𝗎𝗍𝗈 𝖣𝗈𝗐𝗇 𝖥𝖺𝖼𝖾𝖻𝗈𝗈𝗄 𝖵𝗂𝖽𝖾𝗈\n\nchurchill 𝑨𝒔𝒔𝒊𝒔𝒕𝒂𝒏𝒕", attachment: fs.createReadStream(fbvid) }, event.threadID, () => fs.unlinkSync(fbvid));
																}
																catch (e) {
